...
function extractXmpFromXml(xml: string): Extracted {
	...
	try {
		...
		if (m && m[1]) {
			...
			).map((mm) => decodeXmlEntities(mm[1].trim()));
		...
		if (cmtMatch && cmtMatch[1])
			out.comment = decodeXmlEntities(cmtMatch[1].trim());
	} catch {}
	return out;
}
...
function mapFieldsByMime(
	extracted: Extracted,
	mime: string,
): { caption?: string; alternativeText?: string } {
...
async function extractViaFfprobe(absPath: string): Promise<Extracted> {
	try {
		...
		const out: Extracted = {};
		if (
			typeof (tags as any).comment === "string" &&
			(tags as any).comment.trim()
		)
			out.comment = (tags as any).comment.trim();
		if (
			typeof (tags as any).description === "string" &&
			(tags as any).description.trim()
		)
			out.description = (tags as any).description.trim();
		if (
			!out.comment &&
			typeof (tags as any).title === "string" &&
			(tags as any).title.trim()
		)
			out.comment = (tags as any).title.trim();
		return out;
	} catch {
		return {};
	}
		...
		while (p + 4 <= len) {
			...
			if (marker === 0xed /* APP13 */) {
				...
				if (
					...
					while (q + 12 <= segEnd) {
						if (!buf.slice(q, q + 4).equals(Buffer.from("8BIM", "ascii")))
							break;
						q += 4;
						if (q + 2 > segEnd) break;
	...
	while (p + 5 <= len) {
		...
		if (record === 2) {
			if (dataset === 5) {
				...
				// Copyright Notice sometimes used for comments
...
export default {
	register() {
		try {
			...
			const cacheEntry =
				require.cache && require.cache[require.resolve("sharp")];
			if (cacheEntry && !(sharpMod as any).__kajibiPatched) {
				...
				const patched = function patchedSharp(input?: any, options?: any) {
					...
					try {
						if (instance && typeof instance.withMetadata === "function")
		...
		strapi.db.lifecycles.subscribe({
			...
			async afterCreate(event: any) {
				try {
					...
					const isZip =
						(file.mime || "").toLowerCase().includes("zip") ||
					...
					const publicDir =
						(strapi.dirs && (strapi.dirs as any).public) ||
					...
					if (needsAlt && mapped.alternativeText)
						data.alternativeText = mapped.alternativeText;
					...
					if (Object.keys(data).length) {
						...
						try {
							strapi.log.debug(
								`[media-meta] Set fields for ${file.name || file.hash} (${file.mime}): ${JSON.stringify(data)}`,
							);
						} catch {}
					} else {
						try {
							strapi.log.debug(
								`[media-meta] No metadata mapped for ${file.name || file.hash} (${file.mime}). Extracted=${JSON.stringify(extracted)}`,
							);
						} catch {}
					}
				} catch (err) {
...
async function instagramImportFromZip(strapi: any, file: any) {
	try {
		...
		const absPath = path.join(
			publicDir,
			zipUrl.startsWith("/") ? zipUrl.slice(1) : zipUrl,
		);
		...
		const execFileAsync = promisify(execFile);
		try {
			await execFileAsync("unzip", ["-qq", absPath, "-d", tmpBase], {
				timeout: 60000,
			});
		} catch (e) {
			try {
				strapi.log.error(
					`[media-meta] unzip failed for ${file.name}: ${(e as any)?.message}`,
				);
			} catch {}
			return;
		}
		...
		for (const key of keys) {
			...
			if (!jsonPath) {
				try {
					strapi.log.debug(`[ig-import] json not found for ${key}`);
				} catch {}
				continue;
			}
	...
	try {
		...
		const itemsCandidate = Array.isArray(raw)
			...
			: raw && typeof raw === "object"
				? (Object.values(raw).find((v) => Array.isArray(v)) as
						| any[]
						| undefined)
				: undefined;
		const items: any[] = firstArray(itemsCandidate)
			.slice()
			.sort(
		...
		for (const item of items) {
			...
			const rel: any =
				(item as any).uri ||
				(item as any).path ||
				(item as any)?.media?.[0]?.uri ||
				(item as any)?.attachments?.[0]?.data?.uri;
			const ts: any =
				(item as any).creation_timestamp ||
				(item as any).taken_at ||
				(item as any)?.media?.[0]?.creation_timestamp;
			const title: any =
				(item as any).title ||
				(item as any).caption ||
				(item as any)?.media?.[0]?.title ||
				(item as any)?.string_map_data?.Caption?.value ||
				"";
			if (!rel || !ts) {
				try {
					strapi.log.debug(
			...
			for (const c of candidatesForFile) {
				try {
			...
			if (!srcPath) {
				try {
					strapi.log.debug(`[ig-import] media not found for rel=${relNorm}`);
				} catch {}
				continue;
			}
			...
			const exists = await strapi.entityService.findMany(
				"plugin::upload.file",
				{ filters: { name: targetName }, limit: 1 },
			);
			if (exists && exists[0]) continue;

			const tmpOut = srcPath;
			...
			try {
				...
				const mime = (require("mime-types").lookup(srcPath) ||
					(isVideo ? "video/mp4" : "image/jpeg")) as string;
				const files = {
					path: tmpOut,
					filepath: tmpOut,
					tmpPath: tmpOut,
				...
				try {
					strapi.log.debug(
						`[ig-import] svc upload to folderId=${(folder as any)?.id || "root"}`,
					);
				} catch {}
