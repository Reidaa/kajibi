export default (plugin: any) => {
	try {
		try {
			const ctrlKeys = plugin.controllers
				? Object.keys(plugin.controllers)
				: [];
			const svcKeys = plugin.services ? Object.keys(plugin.services) : [];
			strapi.log.debug(
				`[media-meta] Initializing upload plugin extension; controllers=${JSON.stringify(ctrlKeys)} services=${JSON.stringify(svcKeys)}`,
			);
		} catch {}
		...
		if (svc && typeof svc.getSharpInstance === "function") {
			...
			svc.getSharpInstance = (...args: any[]) => {
				...
				try {
					if (inst && typeof inst.withMetadata === "function")
						return inst.withMetadata();
				} catch {}
				return inst;
			};
			try {
				strapi.log.debug(
					"[media-meta] Patched image-manipulation.getSharpInstance with withMetadata()",
				);
			} catch {}
		}
			...
			svc.convertToFormat = async (...args: any[]) => {
				...
				try {
					if (
						result &&
						result.pipeline &&
						typeof result.pipeline.withMetadata === "function"
					) {
	...
	try {
		...
		const extractors = (() => {
			...
			function extractXmpFromXml(xml: string) {
				...
				try {
					let m = xml.match(
						/<dc:description\b[^>]*>([\s\S]*?)<\/dc:description>/i,
					);
					if (m && m[1]) {
						const inner = m[1];
					...
					if (m && m[1]) {
						...
						const items = Array.from(
							inner.matchAll(/<rdf:li[^>]*>([\s\S]*?)<\/rdf:li>/gi),
						).map((mm: any) => decodeXmlEntities(mm[1].trim()));
			...
			function extractFromJpegIptc(buf: Buffer) {
				try {
					...
					while (p + 4 <= buf.length) {
						...
						p += 2;
						const segStart = p;
						const segEnd = Math.min(p + segLen - 2, buf.length);
						if (marker === 0xed) {
							if (
								segEnd - segStart >= header.length &&
								buf.slice(segStart, segStart + header.length).equals(header)
							) {
								let q = segStart + header.length;
								while (q + 12 <= segEnd) {
									if (!buf.slice(q, q + 4).equals(eight)) break;
									q += 4;
									if (q + 2 > segEnd) break;
									const resId = buf.readUInt16BE(q);
									q += 2;
									if (q >= segEnd) break;
									const nameLen = buf[q];
									q += 1;
									...
									const block = buf.slice(q, q + size);
									q += size;
									if (q % 2 === 1) q += 1;
									if (resId === 0x0404) {
										const iptc = parseIptc(block);
										if (iptc.subject || iptc.description || iptc.comment)
											return iptc;
									}
								}
							}
			...
			function parseIptc(data: Buffer) {
				...
				while (p + 5 <= data.length) {
					...
					let size = data.readUInt16BE(p);
					p += 2;
					if ((size & 0x8000) !== 0) {
						if (p + 4 > data.length) break;
						size = data.readUInt32BE(p);
						p += 4;
					}
					if (p + size > data.length) break;
					const value = data.slice(p, p + size);
					p += size;
					if (record === 2) {
			...
			function extractFromMp4(buf: Buffer) {
				...
				try {
					...
					function rd32(o: number) {
						return buf.readUInt32BE(o);
					}
					function type(o: number) {
						return buf.slice(o, o + 4).toString("latin1");
					}
					function walk(start: number, end: number) {
						...
						while (p + 8 <= end) {
							...
							const s = p + header;
							const e = Math.min(p + size, end);
							if (e <= s) break;
							if (t === "meta") {
								walk(Math.min(s + 4, e), e);
							} else if (t === "moov" || t === "udta" || t === "ilst") {
								walk(s, e);
								...
								while (q + 8 <= e) {
									...
									if (!ds) break;
									const ds1 = q + 8;
									const de = Math.min(q + ds, e);
									if (dt === "data") {
										let payload = ds1 + 8;
										if (ds1 + 12 <= de && rd32(ds1 + 8) === 0)
											payload = ds1 + 16;
										const txt = buf
											.slice(payload, de)
											.toString("utf8")
											.replace(/\0+/g, "")
											.trim();
										if (t === "©cmt" && txt) out.comment = txt;
										if (t === "desc" && txt) out.description = txt;
									}
									q += ds;
								}
							}
			...
			function extractExifFromWebp(buf: Buffer) {
				try {
					...
					while (p + 8 <= buf.length) {
						...
						const dataEnd = Math.min(dataStart + size, buf.length);
						if (type === "EXIF") {
							return parseTiff(buf.slice(dataStart, dataEnd));
						}
						p = dataStart + size + (size % 2);
					}
				} catch {}
			...
			function parseTiff(data: Buffer) {
				...
				const le = endian === "II";
				const rd16 = (o: number) =>
					le ? data.readUInt16LE(o) : data.readUInt16BE(o);
				const rd32 = (o: number) =>
					le ? data.readUInt32LE(o) : data.readUInt32BE(o);
				...
				const parseIfd = (off: number) => {
					...
					for (let i = 0; i < count; i++) {
						...
						if (valueBytes <= 4) {
							...
							valueBuf = data.slice(off2, off2 + valueBytes);
						}
						if (tag === 0x010e) {
							const text = decodeExifString(valueBuf)
								.replace(/\0+$/g, "")
								.trim();
			...
			function decodeExifString(buf2: Buffer) {
				try {
					const utf = buf2.toString("utf8");
					if (/\uFFFD/.test(utf) || /Ã.|Â./.test(utf))
						return buf2.toString("latin1");
					return utf;
				} catch {
					return buf2.toString("latin1");
				}
			...
			function mapFieldsByMime(ex: any, mime: string) {
				...
				if (lower.includes("mp4"))
					return { caption: ex.comment, alternativeText: ex.description };
				if (
					lower.includes("webp") ||
					lower.includes("jpeg") ||
					lower.includes("jpg")
				)
					return { caption: ex.subject, alternativeText: ex.description };
				return {};
			}
			return {
		...
		if (plugin.services?.upload?.upload) {
			try {
				strapi.log.debug(
					"[media-meta] Wrapping services.upload.upload for pre-extraction",
				);
			} catch {}
			const origSvcUpload = plugin.services.upload.upload.bind(
				plugin.services.upload,
			);
			plugin.services.upload.upload = async (...args: any[]) => {
				try {
					strapi.log.debug("[media-meta] services.upload.upload called");
				} catch {}
				let files: any[] = [];
				let data: any = {};
				if (
					...
					(args[0].files || args[0].data)
				) {
					files = Array.isArray(args[0].files)
						? args[0].files
						: [args[0].files].filter(Boolean);
					data = args[0].data || {};
				...
				try {
					strapi.log.debug(
						`[media-meta] Found ${files.length} incoming file(s) in service`,
					);
				} catch {}
				const processOne = async (f: any) => {
					try {
						const tmpPath =
							...
							f.path ||
							f.tempFilePath ||
							(f.files && f.files.path);
						let mime = (f.mimetype || f.type || "").toLowerCase();
						const name = (
							f.originalFilename ||
							f.name ||
							(tmpPath && path.basename(tmpPath)) ||
							""
						).toLowerCase();
						if (!mime) {
						...
						if (!tmpPath || !fs.existsSync(tmpPath)) {
							try {
								strapi.log.debug(
									`[media-meta] Skip file (no tmp path): ${name}`,
								);
							} catch {}
							return;
						}
						...
						if (mime.includes("jpeg") || mime.includes("jpg")) {
							const iptc = extractors.extractFromJpegIptc(buf);
							const xmp = extractors.extractXmpFromBuffer(buf);
							extracted = { ...xmp, ...iptc };
						} else if (mime.includes("webp")) {
							const chunks = extractors.listWebpChunks(buf);
							try {
								strapi.log.debug(
									`[media-meta] [svc] WEBP chunks: ${JSON.stringify(chunks.map((c: any) => c.type))}`,
								);
							} catch {}
							...
							let scan: any = {};
							if (
								(!xmp || (!xmp.subject && !xmp.description)) &&
								(!exif || (!exif.subject && !exif.description))
							) {
							...
							if (!extracted.subject && !extracted.description) {
								try {
									...
									try {
										strapi.log.debug(
											`[media-meta] [svc] exiftool failed: ${e?.message}`,
										);
									} catch {}
								}
							}
						} else if (mime.includes("mp4")) {
							const mp4 = extractors.extractFromMp4(buf);
							extracted = { ...mp4 };
						} else {
						...
						try {
							strapi.log.debug(
								`[media-meta] Pre-map ${name} (${mime}) => ${JSON.stringify(mapped)} from ${JSON.stringify(extracted)}`,
							);
						} catch {}
						if (mapped.caption && !f.caption) f.caption = mapped.caption;
						if (mapped.alternativeText && !f.alternativeText)
							f.alternativeText = mapped.alternativeText;
					} catch {}
				};
				...
				try {
					strapi.log.debug("[media-meta] services.upload.upload finished");
				} catch {}
				return res;
			};
		}
		...
		const wrapController = (ctrlName: string, actionName: string) => {
			...
			ctrl[actionName] = async (ctx: any, next?: any) => {
				try {
					strapi.log.debug(
						`[media-meta] controller ${ctrlName}.${actionName} called`,
					);
				} catch {}
				...
				else if (filesObj.files) {
					Array.isArray(filesObj.files)
						? list.push(...filesObj.files)
						: push(filesObj.files);
				} else if (filesObj.file) {
					Array.isArray(filesObj.file)
						? list.push(...filesObj.file)
						: push(filesObj.file);
				} else if (typeof filesObj === "object") {
					Object.values(filesObj).forEach((v: any) => {
						Array.isArray(v) ? list.push(...v) : push(v as any);
					});
				}
				...
				for (const f of list) {
					try {
						...
						if (tmpPath && fs.existsSync(tmpPath)) {
							...
							try {
								buf = Buffer.isBuffer((f as any).buffer)
									? (f as any).buffer
									: Buffer.from((f as any).buffer);
							} catch {}
						}
						try {
							strapi.log.debug(
								`[media-meta] [ctrl] Candidate ${name} (${mime}) tmpPath=${!!tmpPath} bufLen=${buf ? buf.length : 0}`,
							);
						} catch {}
						if (!buf) {
							try {
								strapi.log.debug(
									`[media-meta] [ctrl] Skip (no buffer): ${name}`,
								);
							} catch {}
							continue;
						}
						...
						if (mime.includes("jpeg") || mime.includes("jpg")) {
							...
							try {
								strapi.log.debug(
									`[media-meta] [ctrl] WEBP chunks: ${JSON.stringify(chunks.map((c: any) => c.type))}`,
								);
							} catch {}
							const exif = extractors.extractExifFromWebp(buf);
							const xmp = extractors.extractXmpFromWebp(buf);
								...
								try {
									...
									if (Array.isArray(arr) && arr[0]) {
										...
										const desc =
											...
											meta["ImageDescription"];
										if (Array.isArray(subj))
											extracted.subject = (subj as any[]).join(", ");
										else if (typeof subj === "string") extracted.subject = subj;
										if (typeof desc === "string") extracted.description = desc;
										try {
											strapi.log.debug(
												`[media-meta] [ctrl] exiftool WEBP subj=${JSON.stringify(subj)} desc=${JSON.stringify(desc)}`,
											);
										} catch {}
									}
								} catch (e: any) {
									try {
										strapi.log.debug(
											`[media-meta] [ctrl] exiftool failed: ${e?.message}`,
										);
									} catch {}
								}
							}
						...
						try {
							strapi.log.debug(
								`[media-meta] [ctrl] Pre-map ${name} (${mime}) => ${JSON.stringify(mapped || {})} from ${JSON.stringify(extracted || {})}`,
							);
						} catch {}
					} catch {}
				}
				...
				try {
					...
					try {
						strapi.log.error(
							`[media-meta] controller ${ctrlName}.${actionName} error: ${e?.message}`,
						);
					} catch {}
					throw e;
				}
				try {
					...
					const files = Array.isArray(data)
						? data
						: data && data.length === undefined && data.id
							? [data]
							: [];
					for (const created of files) {
						try {
							...
							const match =
								premap.find((m) => m.name === created.name) ||
								premap.find((m) => name && m.name && name.includes(m.name));
							if (
								!match ||
								(!match.mapped.caption && !match.mapped.alternativeText)
							)
								continue;
							const patch: any = {};
							if (!created.caption && match.mapped.caption)
								patch.caption = match.mapped.caption;
							if (!created.alternativeText && match.mapped.alternativeText)
								patch.alternativeText = match.mapped.alternativeText;
		...
		for (const [c, a] of candidates) {
			...
			if (ok) {
				wrappedAny = true;
				try {
					strapi.log.debug(`[media-meta] Wrapped controller ${c}.${a}`);
				} catch {}
			}
		}
		if (!wrappedAny) {
			...
			if (ctrl) {
				for (const key of Object.keys(ctrl)) {
					if (typeof ctrl[key] === "function") {
						...
						try {
							strapi.log.debug(
								`[media-meta] Wrapped controller admin-upload.${key}`,
							);
						} catch {}
					}
				}
