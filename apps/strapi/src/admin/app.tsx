...
function parseTiff(data: Uint8Array): Extracted {
	...
	const le = endian === "II";
	const rd16 = (o: number) =>
		le ? data[o] | (data[o + 1] << 8) : (data[o] << 8) | data[o + 1];
	const rd32 = (o: number) =>
		le
...
function fillInputsInModal(
	index: number,
	values: { caption?: string; alternativeText?: string },
) {
	...
	const candidates = [
		[
			`input[name="assets.${index}.caption"], textarea[name="assets.${index}.caption"]`,
			values.caption,
		],
		[
			`input[name="assets.${index}.alternativeText"], textarea[name="assets.${index}.alternativeText"]`,
			values.alternativeText,
		],
	] as const;
	...
	if (index === 0) {
		const allCaptions = modal.querySelectorAll<
			HTMLInputElement | HTMLTextAreaElement
		>('input[name$=".caption"], textarea[name$=".caption"]');
		const allAlts = modal.querySelectorAll<
			HTMLInputElement | HTMLTextAreaElement
		>('input[name$=".alternativeText"], textarea[name$=".alternativeText"]');
		...
		for (const m of mutations) {
			for (const node of Array.from(m.addedNodes)) {
				...
				const handleFiles = async (filesList: FileList | File[]) => {
					...
					console.debug(
						"[media-meta] files detected in modal:",
		...
		try {
			...
			try {
				...
				app.addMenuLink({
					...
					icon: CameraIcon,
					intlLabel: {
						id: "instagram-import.menu.label",
						defaultMessage: "Instagram Import",
					},
					Component: async () => {
						...
						const Page = () => {
							...
							const onSubmit = async (e: any) => {
								...
								try {
									...
									let token = "";
									try {
										token = w.strapi?.admin?.auth?.getToken?.() || "";
									} catch {}
									if (!token) {
										try {
											token = w.strapi?.token?.getToken?.() || "";
										} catch {}
									}
							...
							return React.createElement(
								...
								React.createElement(
									"p",
									null,
									"Upload your Instagram archive .zip. The server will process posts, stories, and reels.",
								),
								React.createElement(
									"form",
									{ onSubmit },
									...
									React.createElement(
										"button",
										{ type: "submit", style: { marginLeft: 12 } },
										"Upload zip",
									),
								),
