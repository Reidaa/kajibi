		...
		queryFn: async (): Promise<ListDetailData | null> => {
			...
			const articlesArr = Array.isArray(attrs.articles)
				...
				: Array.isArray(attrs.articles?.data)
					? attrs.articles.data.map((d: any) =>
							d.attributes ? { ...d.attributes, id: d.id } : d,
						)
					: [];
			return {
				...
				category:
					attrs.category?.name || attrs.category?.data?.attributes?.name,
				author: attrs.author?.name || attrs.author?.data?.attributes?.name,
				cover: getMediaUrl(attrs.cover),
				media: (Array.isArray(attrs.media?.data)
					? attrs.media.data
					...
					.map((f: any) => {
						...
						const type: "image" | "video" = String(mime)
							.toLowerCase()
							.includes("video")
							? "video"
							: "image";
				...
				articles: (articlesArr || []).map((a: any) => ({
					...
					title: a.title,
					slug: a.slug,
					media: (Array.isArray(a.media?.data)
						? a.media.data
