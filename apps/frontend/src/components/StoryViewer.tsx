	...
	const goToPreviousPanel = useCallback(() => {
		if (currentPanelIndex > 0) {
			...
			setCurrentPanelIndex(
				sampleStories[currentStoryIndex - 1].panels.length - 1,
			);
		}
	...
	return (
		...
		>
			...
			<button
				...
				className="-translate-y-1/2 absolute top-1/2 left-2 z-30 rounded-full bg-black/30 p-2 text-white transition-all duration-200 hover:bg-black/50 md:left-4"
			...
			<button
				...
				className="-translate-y-1/2 absolute top-1/2 right-2 z-30 rounded-full bg-black/30 p-2 text-white transition-all duration-200 hover:bg-black/50 md:right-4"
				aria-label="Next"
			>
			...
			{/* Story Info */}
			<div className="absolute right-0 bottom-0 left-0 z-20 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-6">
				<div className="text-white">
					<p className="mb-1 text-sm opacity-80">
						Story {currentStoryIndex + 1} of {sampleStories.length}
					</p>
					<h1 className="mb-2 font-bold text-2xl">{currentStory.title}</h1>
