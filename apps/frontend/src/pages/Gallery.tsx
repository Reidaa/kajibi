import { List as ListIcon, Map } from "lucide-react";
...
const GalleryPage = () => {
	...
	const baseStories = !stories
		...
		: listParam
			? stories.filter((s) =>
					(s.lists || []).some((l) => (l.slug || l.id) === listParam),
				)
			: stories;
	...
	const { filtered, matchedPanelByStory, scores } = useSearchFilter(
		baseStories,
		q,
		fields,
	);
						...
						<h2 className="font-bold text-2xl">
							...
							{q && (
								<span className="ml-2 text-base text-muted-foreground">
									({displayedStories.length} résultat
									{displayedStories.length !== 1 ? "s" : ""})
								</span>
							)}
						</h2>
						<button
